            ...
            for (let i = 1; i <= 3; i++) {
                try {
                    const response = await fetch(
                        `https://storage.googleapis.com/griddev/src/assets/images/banner/${i}/json.json`
                    );
                    if (response) {
...
const StyledDiv = styled.div<{ index: number; hasHref?: boolean }>`
    ...
    background-image: ${(props) =>
        `url(https://storage.googleapis.com/griddev/src/assets/images/banner/${props.index}/image.jpg)`};
